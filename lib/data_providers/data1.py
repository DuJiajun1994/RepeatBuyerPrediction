...
class Data1(DataProvider):
    def __init__(self):
        ...
        self.train_size = int(len(train_val_list) * 0.7)
        self.val_size = len(train_val_list) - self.train_size
        self._train_list = train_val_list[:self.train_size]
        self._val_list = train_val_list[self.train_size:]
        ...
        self._user_data = np.loadtxt(user_data_path)
        self._merchant_data = np.loadtxt(merchant_data_path)
    ...
    def _parse_feature(id_str, vector_length):
        ...
        for id_cnt in id_arr:
            ...
            id_ = int(id_cnt_arr[0])
            cnt = int(id_cnt_arr[1])
            assert 0 < id_ <= vector_length
            feature[id_-1] += cnt
        return feature
    ...
    def _get_pair_data(self, df, batch_ids):
        ...
        pair_data = np.vstack([
            np.hstack([
                self._parse_feature(df[feature_name][index_id], feature_length[feature_name]) for feature_name in feature_names
    ...
    def next_batch(self, batch_size, phase):
        ...
        if phase == 'train':
            ...
            batch_data = self._get_batch_data(self._train_df, batch_ids)
        elif phase == 'val':
        ...
        #     batch_data = self._get_batch_data(self._test_df, batch_ids)
        return batch_data
